...
export class BlMultiAttachmentSampleComponent implements SampleAbstractComponent<BLFileFormGroupe>, OnInit {
  ...
  constructor(private fb: UntypedFormBuilder, private toasterService: ToasterService, private translate: TranslateService) {
  ...
  ngOnInit(): void {
    ...
    this.filesList2.push(new FileInfo('fileXX.pdf', 250 * 1024, new Date(), ''))
    ...
    setTimeout(() => {
     ...
     this.formGroup2.controls.attachmentsList.setValue(this.filesList2);
    }, 3000);
  ...
  onDelete(value) {
    ...
    this.toasterService.success(msg.concat(String(' ' + value.name)));
  }
}
