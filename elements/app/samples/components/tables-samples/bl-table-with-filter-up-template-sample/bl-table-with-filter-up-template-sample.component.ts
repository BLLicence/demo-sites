...
import { BlAction, BlComponentConfig, BlDataTableFilters, BlTableComponent, BlTableConfig, BlTableSource, IconClassEnum, ImageInfo } from '@esedit-md/shared-ui';
...
import {BlSampleDialogVrComponent} from "../bl-example-table-sample/bl-sample-dialog-vr/bl-sample-dialog-vr.component";
import {
    ACTION_DEACTIVATE,
    ACTION_DELETE,
    ACTION_NOTIFY,
...
@Component({
  selector: 'bl-table-with-filter-up-template-sample',
  templateUrl: 'bl-table-with-filter-up-template-sample.component.html',
})
export class BlTableWithFilterUpTemplateSampleComponent implements OnInit {
  ...
  constructor(private toasterService: ToasterService,
      ...
      this.filterComponentsConfig.push({fieldName: "id", label: this.translateService.instant('sample.datatable.header.matricule')});
      this.filterComponentsConfig.push({fieldName: "nom", label: this.translateService.instant('sample.datatable.header.lastname')});
  ...
  ngOnInit() {
    this.pageTitle = this.translateService.instant(
      'pages.templates.table-with-filter-up.sampleTitle'
    );
  }
  ...
  private getGroupedActionsFirstTable(){
      ...
      return [ this.actionDeactivate,this.openAction];
  }
...
private openFilter() {
  ...
  resetEvent(table: BlTableComponent) {
    this.formGroup2.reset();
    this.searchEvent(table);
  }
}
