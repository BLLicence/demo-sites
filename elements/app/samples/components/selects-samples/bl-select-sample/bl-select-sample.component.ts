  ...
  implements OnInit
{
  ...
  ngOnInit() {
    this.formGroup = new FormGroup<any>({
      listBox: new FormControl<BlBasicObject | null>(this.values[0],[this.validatorRequired()]),
    ...
    this.customErrorsMap.set('requiredError','pages.variables.form-error.customErrorMessage.bl-select.required');
  ...
  private validatorRequired(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      ...
      if (selectedValue && selectedValue.length!=0) {
        ...
        return {'requiredError': true}; // Validation failed
      }
    };
  }
}
